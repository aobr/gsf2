...
* Python source files:
  * The Python package source files are located in the `gsf` directory.
  * The file `gsf.__main__.py` defines a command line interface that
    is accessible both as the command `gsf` and via
    `python -m gsf`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_gsf.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `gsf`.
